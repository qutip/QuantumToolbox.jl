...
struct EnrSpace{N} <: AbstractSpace
    ...
    function EnrSpace(
            ...
            n_excitations::Int;
            excitation_weights::AbstractVecOrTuple{T} = ntuple(_ -> 1, length(dims)),
        ) where {T <: Integer}
...
function enr_fock(
        ...
        excitation_weights::AbstractVecOrTuple{Td} = ntuple(_ -> 1, length(dims)),
...
function enr_thermal_dm(
        ...
        sparse::Union{Bool, Val} = Val(false),
        excitation_weights::AbstractVecOrTuple{T1} = ntuple(_ -> 1, length(dims)),
