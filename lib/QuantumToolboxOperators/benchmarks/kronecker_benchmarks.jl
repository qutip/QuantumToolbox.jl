...
K1 = KroneckerOperator(dims, 1 => a1, 2 => a2, 3 => a3, 4 => a4)
K1c = cache_operator(K1, v)

S1 = kron(a1, a2, a3, a4)
S1c = cache_operator(S1, v)

println("  KroneckerOperator memory: $(Base.summarysize(K1c)) bytes")
println("  SciML TensorProduct memory: $(Base.summarysize(S1c)) bytes")
println("  Memory ratio (SciML / Kron): $(round(Base.summarysize(S1c) / Base.summarysize(K1c); digits=1))x")
println()
...
K2 = KroneckerOperator(dims, 1 => a1, 4 => a4)
K2c = cache_operator(K2, v)

S2 = reduce(kron, [a1, I2, I3, a4])
S2c = cache_operator(S2, v)

println("  KroneckerOperator memory: $(Base.summarysize(K2c)) bytes")
println("  SciML TensorProduct memory: $(Base.summarysize(S2c)) bytes")
println("  Memory ratio (SciML / Kron): $(round(Base.summarysize(S2c) / Base.summarysize(K2c); digits=1))x")
println()
...
K3 = KroneckerOperator(dims, 1 => a1, 3 => a3)
K3c = cache_operator(K3, v)

S3 = reduce(kron, [a1, I2, a3, I4])
S3c = cache_operator(S3, v)

println("  KroneckerOperator memory: $(Base.summarysize(K3c)) bytes")
println("  SciML TensorProduct memory: $(Base.summarysize(S3c)) bytes")
println("  Memory ratio (SciML / Kron): $(round(Base.summarysize(S3c) / Base.summarysize(K3c); digits=1))x")
println()
...
K4 = KroneckerOperator(dims, 4 => a4)  # physics mode 4 → julia dim 1 (fast path)
K4c = cache_operator(K4, v)

S4 = reduce(kron, (I1, I2, I3, a4)) # kron(I1, I2, I3, a4)
S4c = cache_operator(S4, v)

println("  KroneckerOperator memory: $(Base.summarysize(K4c)) bytes")
println("  SciML TensorProduct memory: $(Base.summarysize(S4c)) bytes")
println("  Memory ratio (SciML / Kron): $(round(Base.summarysize(S4c) / Base.summarysize(K4c); digits=1))x")
println()
