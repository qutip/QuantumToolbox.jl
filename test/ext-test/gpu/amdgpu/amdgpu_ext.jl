...
@testset "AMDGPU Correlations and Spectrum" begin
    N = 10
    Id = qeye(N)
    a = destroy(N) |> rocSPARSE.ROCSparseMatrixCSR
    H = a' * a
...
@testset "AMDGPU eigsolve" begin
    ...
    vals_gpu, vecs_gpu = eigenstates(
        ...
        eigvals = 4,
        krylovdim = 30,
        solver = KrylovJL_GMRES(), # No direct (CUDSS-like) GPU solver available for AMDGPU
        v0 = normalize!(AMDGPU.rand(ComplexF64, size(L_gpu, 1))),
    )
    ...
    vals_al_gpu, vecs_al_gpu = eigsolve_al(
        ...
        ρ0 = roc(operator_to_vector(rand_dm(N))),
    )
