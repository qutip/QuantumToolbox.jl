@testitem "States and Operators" begin
    ...
    @testset "Element types" begin
        ...
        for FT in float_type_list
            ...
            @test CT == eltype(displace(N, rand(FT)))
            @test CT == eltype(squeeze(N, rand(FT)))
        ...
        @test ComplexF64 == eltype(coherent(N, 1))
        @test ComplexF64 == eltype(coherent_dm(N, 1))
        @test Float64 == eltype(thermal_dm(N, 1; sparse = Val(true)))
        @test Float64 == eltype(thermal_dm(N, 1; sparse = Val(false)))
        @test ComplexF64 == eltype(displace(N, 1))
        @test ComplexF64 == eltype(squeeze(N, 1))
