@testitem "States and Operators" begin
    ...
    @testset "thermal state" begin
        ...
        ρTd = thermal_dm(Complex{BigFloat}, N, big(0.123))
        ρTs = thermal_dm(Complex{BigFloat}, N, big(0.123); sparse = Val(true))
        @test isoper(ρTd)
        @test ρTd.dims == [N]
    ...
    @testset "Element types" begin
        ...
        for T in type_list
            ...
            @test T == eltype(thermal_dm(T, N, rand(Float32); sparse = Val(true)))
            @test T == eltype(thermal_dm(T, N, rand(Float32); sparse = Val(false)))
            @test T == eltype(maximally_mixed_dm(T, N))
            @test T == eltype(rand_dm(T, N))
            @test T == eltype(spin_state(T, 0.5, 0.5))
            @test T == eltype(spin_coherent(T, 0.5, rand(Float32), rand(Float32)))
            @test T == eltype(bell_state(T, 0, 0))
            @test T == eltype(singlet_state(T))
            @test all(==(T), eltype.(triplet_states(T)))
            @test T == eltype(w_state(T, N))
            ...
            @test T == eltype(spin_Jm(T, 0.5))
            @test T == eltype(spin_Jp(T, 0.5))
            @test all(==(T), eltype.(spin_J_set(T, 0.5)))
            @test T == eltype(sigmam(T))
            @test T == eltype(sigmap(T))
            @test T == eltype(sigmax(T))
            @test T == eltype(sigmay(T))
            @test T == eltype(sigmaz(T))
            @test T == eltype(destroy(T, N))
            @test T == eltype(create(T, N))
            @test T == eltype(eye(T, N))
            @test T == eltype(projection(T, N, 0, 0))
            @test T == eltype(displace(T, N, rand(Float32)))
            @test T == eltype(squeeze(T, N, rand(Float32)))
            @test T == eltype(num(T, N))
