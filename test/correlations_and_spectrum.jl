@testset "Correlations and Spectrum" begin
    ...
    ω_l1, spec1 =
        spectrum(H, ω_l, a', a, c_ops, solver = FFTCorrelation(), progress_bar = false)
    ...
    @test sum(abs2.(spec1[idxs1] .- test_func1[idxs1])) / sum(abs2.(test_func1[idxs1])) <
          0.01
    @test sum(abs2.(spec2[idxs2] .- test_func2[idxs2])) / sum(abs2.(test_func2[idxs2])) <
          0.01
end