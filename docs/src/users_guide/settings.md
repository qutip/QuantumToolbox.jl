...
All settings are stored in [`QuantumToolbox.settings`](@ref QuantumToolboxCore.settings).
...
First, we can check the current [`QuantumToolbox.settings`](@ref QuantumToolboxCore.settings):
