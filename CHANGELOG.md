...
- Guard time-dependent `sesolve`/`mesolve` evolutions with `dtmax` (the largest gap in `tlist`) instead of forcing `tstops = tlist`. Both prevent the integrator from stepping over a feature of the coefficient functions after a long quiet stretch, but an exact stop at every output point also resets the step-size controller: for a driven-dissipative oscillator with 100 output points the old default needed 2661 RHS evaluations and 40 rejected steps versus 2175 and 3 now, making time-dependent solves ~1.3-1.4x faster with unchanged accuracy. Protection is unchanged on pulsed problems (verified against Gaussian and rectangular features from 5x down to 0.25x the output spacing). A user-supplied `dtmax` now takes precedence, and a user-supplied `tstops` is passed through untouched instead of being unioned with `tlist`, so an explicit `tstops` can be used for a coefficient discontinuity that `tlist` does not resolve. `ssesolve`/`smesolve` are unaffected: they still need `tstops = tlist` when `store_measurement = true`. ([#750])

- Guard time-dependent `sesolve`/`mesolve` against stepping over a feature of the coefficient functions with `dtmax` (the largest gap in `tlist`) instead of forcing `tstops = tlist`. Both prevent the integrator from growing `dt` large enough to jump over a pulse that switches on after a quiet stretch, but forcing an exact stop at every output point also resets the step-size controller: for a driven-dissipative oscillator with 100 output points the old behaviour needed 2661 RHS evaluations and 40 rejected steps, against 2175 and 3 now. On pulse tests (Gaussian and rectangular, widths above/at/below the output spacing) both policies resolve every feature the output grid can represent, and `dtmax` gets there with 12-26% fewer RHS evaluations. A user-supplied `tstops` is now passed through untouched rather than unioned with `tlist`, so an explicit `tstops` (for a discontinuity the output grid does not resolve) or an explicit `dtmax` can override the default. `ssesolve`/`smesolve` are unchanged: they need `tstops = tlist` to reconstruct `dW/dt`, not as a step-size guard. ([#750])

- Use `InterpolatingAdjoint(checkpointing = true)` instead of `BacksolveAdjoint` for the reverse-mode `mesolve` benchmarks, tests and documentation. Lindblad dynamics is contracting, so `BacksolveAdjoint` integrates an expanding system backwards; its checkpoints default to the saved time points, which makes gradient accuracy depend on `length(tlist)` rather than on the requested tolerance. For the documented example the reverse-mode gradient is correct with 100 output points, but has a relative error of `2e-4` with 50 points, `3e3` with 25, and diverges to `~1e30` with 10 — silently, without an error or a `NaN`. `InterpolatingAdjoint(checkpointing = true)` never re-integrates the state backwards and stays accurate (`~6e-8`) for every grid tested, while also being ~1.6x faster on both Mooncake and Enzyme. `sesolve` keeps `BacksolveAdjoint`: its dynamics is unitary and reverses stably, and there the reverse solve is the accurate one, whereas `InterpolatingAdjoint` interpolates the (non-dense, since `save_everystep = false`) forward solution and is 9% off at 100 output points. ([#750])
...
- Document that the default `save_everystep = false` leaves interpolation-based adjoints (`InterpolatingAdjoint`, `GaussAdjoint`, `QuadratureAdjoint`) without the dense forward solution they interpolate, so they silently lose accuracy. For `sesolve` this is a 9% gradient error at 100 output points, removed entirely by passing `save_everystep = true`; `mesolve` is unaffected because its dynamics relaxes. ([#750])
