...
function ssesolveProblem(
    ...
    K_l = sum(
        op ->
            _ScalarOperator_e(op, +) * op +
            _ScalarOperator_e2_2(op, -) * IdentityOperator(hilbert_dimensions_to_size(dims)[1]),
        sc_ops_evo_data,
    )
    ...
    D_l = map(
        op -> op + _ScalarOperator_e(op, -) * IdentityOperator(hilbert_dimensions_to_size(dims)[1]),
        sc_ops_evo_data,
    )
