...
mesolve_td_dudt!(du, u, p, t) = mul!(du, p.L + p.H_t(t).data, u)
...
function mesolveProblem(H::QuantumObject{<:AbstractArray{T1},HOpType},
    ...
    tspan = (t_l[1], t_l[end])
    _mesolveProblem(L, ρ0, tspan, alg, Val(is_time_dependent), p; kwargs2...)
end

function _mesolveProblem(L::AbstractMatrix{<:T1}, ρ0::AbstractVector{<:T2}, tspan::Tuple, alg::OrdinaryDiffEq.OrdinaryDiffEqAlgorithm, is_time_dependent::Val{false}, p; kwargs...) where {T1,T2}
    ODEProblem{true,SciMLBase.FullSpecialize}(mesolve_ti_dudt!, ρ0, tspan, p; kwargs...)
end

function _mesolveProblem(L::AbstractMatrix{<:T1}, ρ0::AbstractVector{<:T2}, tspan::Tuple, alg::OrdinaryDiffEq.OrdinaryDiffEqAlgorithm, is_time_dependent::Val{true}, p; kwargs...) where {T1,T2}
    ODEProblem{true,SciMLBase.FullSpecialize}(mesolve_td_dudt!, ρ0, tspan, p; kwargs...)
end
