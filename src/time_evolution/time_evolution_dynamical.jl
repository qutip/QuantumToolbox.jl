...
function dfd_mesolveProblem(H::Function, ψ0::QuantumObject{<:AbstractArray{T1},StateOpType},
    ...
    params2 = merge(params, (H_fun = H, c_ops_fun = c_ops, e_ops_fun = e_ops,
                        ...
                        dfd_ρt_cache = similar(ψ0.data, length(ψ0.data)^2), dfd_params = dfd_params))

    cb_dfd = DiscreteCallback(_DFDIncreaseReduceCondition, _DFDIncreaseReduceAffect!, save_positions=(false, false))
    kwargs2 = (;kwargs...)
    kwargs2 = haskey(kwargs2, :callback) ? merge(kwargs2, (callback = CallbackSet(cb_dfd, kwargs2.callback),)) : merge(kwargs2, (callback = cb_dfd,))

    mesolveProblem(H₀, ψ0, t_l, c_ops₀; e_ops=e_ops₀, alg=alg, H_t=H_t,
...
function dfd_mesolve(H::Function, ψ0::QuantumObject{<:AbstractArray{T1},StateOpType},
    ...
    dfd_prob = dfd_mesolveProblem(H, ψ0, t_l, c_ops, maxdims, dfd_params; alg=alg, e_ops=e_ops, H_t=H_t,
                                    params=params, tol_list=tol_list, kwargs...)
...
function _DSF_mesolve_Affect!(integrator)
    ...
    copyto!(internal_params.L, liouvillian(H(op_l2, dsf_params), c_ops(op_l2, dsf_params), dsf_identity).data)
end
...
function dsf_mesolveProblem(H::Function,
    ...
    kwargs...) where {T,StateOpType<:Union{KetQuantumObject,OperatorQuantumObject},TOl}
    ...
    kwargs2 = (;kwargs...)
    kwargs2 = haskey(kwargs2, :callback) ? merge(kwargs2, (callback = CallbackSet(cb_dsf, kwargs2.callback),)) : merge(kwargs2, (callback = cb_dsf,))

    mesolveProblem(H₀, ψ0, t_l, c_ops₀; e_ops=e_ops₀, alg=alg, H_t=H_t, params=params2, kwargs2...)
end
...
function dsf_mesolve(H::Function,
    ...
    dsf_prob = dsf_mesolveProblem(H, ψ0, t_l, c_ops, op_list, α0_l, dsf_params; alg=alg, e_ops=e_ops, H_t=H_t,
...
function dsf_mesolve(H::Function,
    ...
    return dsf_mesolve(H, ψ0, t_l, c_ops, op_list, α0_l, dsf_params; alg=alg, e_ops=e_ops, H_t=H_t,
                            params=params, δα_list=δα_list, krylov_dim=krylov_dim, kwargs...)
end
...
function dsf_mcsolveEnsembleProblem(H::Function,
    ...
    mcsolveEnsembleProblem(H₀, ψ0, t_l, c_ops₀; e_ops=e_ops₀, alg=alg, H_t=H_t,
                                params=params2, jump_callback=jump_callback, 
...
function dsf_mcsolve(H::Function,
    ...
    ens_prob_mc = dsf_mcsolveEnsembleProblem(H, ψ0, t_l, c_ops, op_list, α0_l, dsf_params; alg=alg, e_ops=e_ops, 
                H_t=H_t, params=params, δα_list=δα_list,
                jump_callback=jump_callback, krylov_dim=krylov_dim, kwargs...)
