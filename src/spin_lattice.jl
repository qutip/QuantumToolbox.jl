...
If all sites have the same Hilbert space dimension (specified by `d`), you can use the second method:

```jldoctest
julia> N = Val(4); # four sites

julia> d = 3;      # each site is a three-level system

julia> a = multisite_operator(N, d, 2=>destroy(d));

julia> a.dims
([3, 3, 3, 3], [3, 3, 3, 3])
```

Another example, since spin-``\frac{1}{2}`` is a two-level system, the argument `d` can be omitted. That is, for an `8`-site spin-``\frac{1}{2}`` chain, the operator ``\hat{\sigma}^x_5 \hat{\sigma}^z_7`` can be generated by
