...
@doc raw"""
    ...
    multisite_operator(Nsite::Union{Int, Val}, site_dims::Integer, pairs::Pair{<:Integer,<:QuantumObject}...)
...
## second method
- `Nsite::Union{Integer, Val}`: The number of sites (`N` or `Val(N)`), and each site is assumed to have the same Hilbert space dimension `site_dims`.
- `site_dims::Integer`: The Hilbert space dimension of each site. If this argument is not provided, it defaults to `2`, which corresponds to a two-level system.
- `pairs::Pair{<:Integer,<:QuantumObject}...`: A list of pairs where the first element of the pair is the site index and the second element is the operator acting on that site.
...
can be generated by the first method as follows:
...
If all sites have the same Hilbert space dimension (specified by `site_dims`), you can use the second method:

```jldoctest
julia> Nsite = Val(4); # four sites

julia> site_dims = 3;  # each site is a three-level system

julia> a = multisite_operator(Val(4), site_dims, 2=>destroy(site_dims));

julia> a.dims
([3, 3, 3, 3], [3, 3, 3, 3])
```

Another example, since spin-``\frac{1}{2}`` is a two-level system, the `site_dims` argument can be omitted. That is, for an `8`-site spin-``\frac{1}{2}`` chain, the operator ``\hat{\sigma}^x_5 \hat{\sigma}^z_7`` can be generated by
...
function multisite_operator(Nsite::Union{Integer, Val}, site_dims::Integer, pairs::Pair{<:Integer, <:QuantumObject}...)
    dims = ntuple(j -> site_dims, makeVal(Nsite))
    return multisite_operator(dims, pairs...)
end
multisite_operator(Nsite::Union{Integer, Val}, pairs::Pair{<:Integer, <:QuantumObject}...) = multisite_operator(Nsite, 2, pairs...)
