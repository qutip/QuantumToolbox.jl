...
spre(A::AbstractQuantumObject{DT,OperatorQuantumObject}, Id_cache = I(size(A, 1))) where {DT} =
    get_typename_wrapper(A)(_spre(A.data, Id_cache), SuperOperator, A._dims)
...
spost(B::AbstractQuantumObject{DT,OperatorQuantumObject}, Id_cache = I(size(B, 1))) where {DT} =
    get_typename_wrapper(B)(_spost(B.data, Id_cache), SuperOperator, B._dims)
...
function sprepost(
    ...
    check_dims(A, B)
    return promote_op_type(A, B)(_sprepost(A.data, B.data), SuperOperator, A._dims)
end
...
lindblad_dissipator(O::AbstractQuantumObject{DT,OperatorQuantumObject}, Id_cache = I(size(O, 1))) where {DT} =
    get_typename_wrapper(O)(_lindblad_dissipator(O.data, Id_cache), SuperOperator, O._dims)
...
liouvillian(H::AbstractQuantumObject{DT,OperatorQuantumObject}, Id_cache::Diagonal = I(prod(H._dims))) where {DT} =
    get_typename_wrapper(H)(_liouvillian(H.data, Id_cache), SuperOperator, H._dims)
