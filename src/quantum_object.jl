...
function QuantumObject(
    ...
    if N == 1
        ...
        throw(
            DomainError(
                Size,
                "The dimension of the array is not compatible with Quantum Object",
            ),
        ) : nothing
    end
...
function _check_QuantumObject(type::KetQuantumObject, prod_dims::Int, m::Int, n::Int)
    (n != 1) ?
    throw(
        DomainError((m, n), "The dimension of the array is not compatible with Ket type"),
    ) : nothing
    prod_dims != m ?
    throw(
        DimensionMismatch("The dims parameter does not fit the dimension of the Array."),
    ) : nothing
end
...
function _check_QuantumObject(type::BraQuantumObject, prod_dims::Int, m::Int, n::Int)
    (m != 1) ?
    throw(
        DomainError((m, n), "The dimension of the array is not compatible with Bra type"),
    ) : nothing
    prod_dims != n ?
    throw(
        DimensionMismatch("The dims parameter does not fit the dimension of the Array."),
...
function _check_QuantumObject(type::OperatorQuantumObject, prod_dims::Int, m::Int, n::Int)
    ...
    throw(
        DomainError(
            (m, n),
            "The dimension of the array is not compatible with Operator type",
        ),
    ) : nothing
    prod_dims != m ?
    throw(
...
function _check_QuantumObject(
    type::SuperOperatorQuantumObject,
    prod_dims::Int,
    m::Int,
    n::Int,
)
    ...
    throw(
        DomainError(
            (m, n),
            "The dimension of the array is not compatible with SuperOperator type",
        ),
    ) : nothing
    prod_dims != sqrt(m) ?
    throw(
...
function _check_QuantumObject(
    ...
    throw(
        DomainError(
            (m, n),
            "The dimension of the array is not compatible with OperatorKet type",
        ),
    ) : nothing
    prod_dims != sqrt(m) ?
    throw(
...
function _check_QuantumObject(
    ...
    throw(
        DomainError(
            (m, n),
            "The dimension of the array is not compatible with OperatorBra type",
        ),
    ) : nothing
    prod_dims != sqrt(n) ?
    throw(
...
"""
isbra(A::QuantumObject{<:AbstractArray{T},OpType}) where {T,OpType<:QuantumObjectType} =
    OpType <: BraQuantumObject
...
"""
isket(A::QuantumObject{<:AbstractArray{T},OpType}) where {T,OpType<:QuantumObjectType} =
    OpType <: KetQuantumObject
...
"""
Base.size(A::QuantumObject{<:AbstractArray{T},OpType}) where {T,OpType<:QuantumObjectType} =
    size(A.data)
Base.size(
    A::QuantumObject{<:AbstractArray{T},OpType},
    inds...,
) where {T,OpType<:QuantumObjectType} = size(A.data, inds...)

Base.getindex(
    A::QuantumObject{<:AbstractArray{T},OpType},
    inds...,
) where {T,OpType<:QuantumObjectType} = getindex(A.data, inds...)
Base.setindex!(
    A::QuantumObject{<:AbstractArray{T},OpType},
    val,
    inds...,
) where {T,OpType<:QuantumObjectType} = setindex!(A.data, val, inds...)
...
Base.length(
    A::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:QuantumObjectType} = length(A.data)

SparseArrays.sparse(
    A::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:QuantumObjectType} = QuantumObject(sparse(A.data), OpType(), A.dims)
SparseArrays.nnz(
    A::QuantumObject{<:AbstractSparseArray,OpType},
) where {OpType<:QuantumObjectType} = nnz(A.data)
SparseArrays.nonzeros(
    A::QuantumObject{<:AbstractSparseArray,OpType},
) where {OpType<:QuantumObjectType} = nonzeros(A.data)
SparseArrays.rowvals(
    A::QuantumObject{<:AbstractSparseArray,OpType},
) where {OpType<:QuantumObjectType} = rowvals(A.data)
SparseArrays.droptol!(
    A::QuantumObject{<:AbstractSparseArray,OpType},
    tol::Real,
) where {OpType<:QuantumObjectType} = (droptol!(A.data, tol); return A)
SparseArrays.dropzeros(
    A::QuantumObject{<:AbstractSparseArray,OpType},
) where {OpType<:QuantumObjectType} = QuantumObject(dropzeros(A.data), A.type, A.dims)
SparseArrays.dropzeros!(
    A::QuantumObject{<:AbstractSparseArray,OpType},
) where {OpType<:QuantumObjectType} = (dropzeros!(A.data); return A)

Base.isequal(
    A::QuantumObject{<:AbstractArray{T},OpType},
    B::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:QuantumObjectType} =
...
function Base.show(
    io::IO,
    QO::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:OperatorQuantumObject}
    op_data = QO.data
    println(
        io,
        "Quantum Object:   type=",
...
for op in (:(+), :(-), :(*))
    @eval begin
        function LinearAlgebra.$op(
            ...
            A.dims != B.dims && throw(
                ErrorException("The two operators are not of the same Hilbert dimension."),
            )
        ...
        ) where {T1<:Number,T2,OpType<:QuantumObjectType} =
            QuantumObject($(op)(n * I, A.data), OpType(), A.dims)
        LinearAlgebra.$op(
            A::QuantumObject{<:AbstractArray{T1},OpType},
            n::T2,
        ) where {T1,T2<:Number,OpType<:QuantumObjectType} =
...
LinearAlgebra.:(^)(
    A::QuantumObject{<:AbstractArray{T},OpType},
    n::T1,
) where {T,T1<:Number,OpType<:QuantumObjectType} =
...
LinearAlgebra.adjoint(
    A::QuantumObject{<:AbstractArray{T},OperatorKetQuantumObject},
) where {T} = QuantumObject(adjoint(A.data), OperatorBra, A.dims)
LinearAlgebra.adjoint(
    A::QuantumObject{<:AbstractArray{T},OperatorBraQuantumObject},
) where {T} = QuantumObject(adjoint(A.data), OperatorKet, A.dims)
...
LinearAlgebra.svdvals(A::QuantumObject{<:AbstractSparseMatrix}) =
...
LinearAlgebra.normalize(
    A::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:QuantumObjectType} = QuantumObject(normalize(A.data), OpType(), A.dims)
LinearAlgebra.normalize!(
    A::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:QuantumObjectType} = (normalize!(A.data); A)
LinearAlgebra.ishermitian(
    A::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:QuantumObjectType} = ishermitian(A.data)
LinearAlgebra.issymmetric(
    A::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:QuantumObjectType} = issymmetric(A.data)
LinearAlgebra.isposdef(
    A::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:QuantumObjectType} = isposdef(A.data)
...
@inline LinearAlgebra.mul!(
    y::AbstractVector{Ty},
    A::QuantumObject{<:AbstractMatrix{Ta}},
    x,
    α,
    β,
) where {Ty,Ta} = mul!(y, A.data, x, α, β)

LinearAlgebra.sqrt(
    A::QuantumObject{<:AbstractArray{T},OpType},
) where {T,OpType<:QuantumObjectType} = QuantumObject(sqrt(A.data), OpType(), A.dims)

LinearAlgebra.exp(
    A::QuantumObject{<:AbstractMatrix{T},OpType},
) where {T,OpType<:QuantumObjectType} =
    QuantumObject(dense_to_sparse(exp(A.data)), OpType(), A.dims)

LinearAlgebra.exp(
    A::QuantumObject{<:AbstractSparseMatrix{T},OpType},
) where {T,OpType<:QuantumObjectType} = QuantumObject(_spexp(A.data), OpType(), A.dims)
...
Base.Vector(A::QuantumObject{<:AbstractVector}) =
    QuantumObject(Vector(A.data), A.type, A.dims)
Base.Vector{T}(A::QuantumObject{<:AbstractVector}) where {T<:Number} =
    QuantumObject(Vector{T}(A.data), A.type, A.dims)
Base.Matrix(A::QuantumObject{<:AbstractMatrix}) =
    QuantumObject(Matrix(A.data), A.type, A.dims)
Base.Matrix{T}(A::QuantumObject{<:AbstractMatrix}) where {T<:Number} =
    QuantumObject(Matrix{T}(A.data), A.type, A.dims)
SparseArrays.SparseVector(A::QuantumObject{<:AbstractVector}) =
    QuantumObject(SparseVector(A.data), A.type, A.dims)
