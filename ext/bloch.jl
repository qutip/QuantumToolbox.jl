...
function QuantumToolbox.render(b::QuantumToolbox.Bloch; location=nothing)
    ...
    sphere_primitive = Makie.Sphere(Makie.Point3f(0, 0, 0), 1f0)
    Makie.mesh!(b.ax, sphere_primitive;
        color = sphere_color,
        transparency = true
    )
    Makie.wireframe!(b.ax, sphere_primitive;
        color = frame_color,
        linewidth = b.frame_width,
        transparency = true
    )

    Makie.lines!(b.ax, [Makie.Point3f(-1, 0, 0), Makie.Point3f(1, 0, 0)];
        color = :black, linewidth = 0.5) # X-axis
    Makie.lines!(b.ax, [Makie.Point3f(0, -1, 0), Makie.Point3f(0, 1, 0)];
        color = :black, linewidth = 0.5) # Y-axis
    Makie.lines!(b.ax, [Makie.Point3f(0, 0, -1), Makie.Point3f(0, 0, 1)];
        color = :black, linewidth = 0.5) # Z-axis

    Makie.text!(b.ax, Makie.Point3f(b.xlpos[1], 0, 0); text = b.xlabel[1],
        align = (:center, :center),
        color = Makie.to_color(b.font_color),
        fontsize = b.font_size)
    Makie.text!(b.ax, Makie.Point3f(b.xlpos[2], 0, 0); text = b.xlabel[2],
        align = (:center, :center),
        color = Makie.to_color(b.font_color),
        fontsize = b.font_size)
    Makie.text!(b.ax, Makie.Point3f(0, b.ylpos[1], 0); text = b.ylabel[1],
        align = (:center, :center),
        color = Makie.to_color(b.font_color),
        fontsize = b.font_size)
    Makie.text!(b.ax, Makie.Point3f(0, b.ylpos[2], 0); text = b.ylabel[2],
        align = (:center, :center),
        color = Makie.to_color(b.font_color),
        fontsize = b.font_size)
    Makie.text!(b.ax, Makie.Point3f(0, 0, b.zlpos[1]), text = b.zlabel[1],
        align = (:center, :center),
        color = Makie.to_color(b.font_color),
        fontsize = b.font_size)
    Makie.text!(b.ax, Makie.Point3f(0, 0, b.zlpos[2]), text = b.zlabel[2],
        align = (:center, :center),
        color = Makie.to_color(b.font_color),
        fontsize = b.font_size)
    ...
    for (i, pnt) in enumerate(b.points)
        ...
        Makie.scatter!(b.ax, [Makie.Point3f(pnt...)];
    ...
    for line_pts in b.lines
        Makie.lines!(b.ax, Makie.Point3f.(line_pts);
            color = :blue, linewidth = 1.0)
    end

    for arc_pts in b.arcs
        Makie.lines!(b.ax, Makie.Point3f.([arc_pts[1], arc_pts[end]]);
