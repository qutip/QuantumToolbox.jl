...
"""
CuArray(A::QuantumObject{Tq}) where {Tq<:Union{Vector,Matrix}} =
    QuantumObject(CuArray(A.data), A.type, A.dims)
...
CuArray{T}(A::QuantumObject{Tq}) where {T,Tq<:Union{Vector,Matrix}} =
    QuantumObject(CuArray{T}(A.data), A.type, A.dims)
...
"""
CuSparseVector(A::QuantumObject{<:SparseVector}) =
    QuantumObject(CuSparseVector(A.data), A.type, A.dims)
...
CuSparseVector{T}(A::QuantumObject{<:SparseVector}) where {T} =
    QuantumObject(CuSparseVector{T}(A.data), A.type, A.dims)
...
"""
CuSparseMatrixCSC(A::QuantumObject{<:SparseMatrixCSC}) =
    QuantumObject(CuSparseMatrixCSC(A.data), A.type, A.dims)
...
"""
CuSparseMatrixCSR(A::QuantumObject{<:SparseMatrixCSC}) =
    QuantumObject(CuSparseMatrixCSR(A.data), A.type, A.dims)
...
cu(
    A::QuantumObject{T},
    word_size::TW,
) where {T<:Union{Vector,Matrix},TW<:Union{Val{32},Val{64}}} =
...
_change_eltype(::Type{Complex{T}}, ::Val{64}) where {T<:Union{Int,AbstractFloat}} =
    ComplexF64
_change_eltype(::Type{Complex{T}}, ::Val{32}) where {T<:Union{Int,AbstractFloat}} =
    ComplexF32
