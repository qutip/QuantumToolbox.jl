...
If `A.data` is a dense array, return a new [`QuantumObject`](@ref) with `A.data` is in the type of `CUDA.CuArray` for gpu calculations.
"""
...
If `A.data` is a sparse vector, return a new [`QuantumObject`](@ref) with `A.data` is in the type of `CUDA.CUSPARSE.CuSparseVector` for gpu calculations.
"""
...
If `A.data` is in the type of `SparseMatrixCSC`, return a new [`QuantumObject`](@ref) with `A.data` is in the type of `CUDA.CUSPARSE.CuSparseMatrixCSC` for gpu calculations.
"""
...
If `A.data` is in the type of `SparseMatrixCSC`, return a new [`QuantumObject`](@ref) with `A.data` is in the type of `CUDA.CUSPARSE.CuSparseMatrixCSR` for gpu calculations.
"""
...
Return a new `QuantumObject` with `A.data` is in the type of `CUDA` arrays for gpu calculations.

# Arguments
- `A::QuantumObject`: [`QuantumObject`](@ref)
